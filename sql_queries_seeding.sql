...
CREATE TABLE messages (
    ...
    userId INT,  -- Référence à l'utilisateur qui envoie le message
    courseId INT,   -- Référence au cours auquel le message est associé
...
CREATE TABLE likes ( id INT AUTO_INCREMENT PRIMARY KEY, userId INT, -- Référence à l'utilisateur qui aime messageId INT DEFAULT NULL, -- Peut être nul si c'est un like sur un cours courseId INT DEFAULT NULL, -- Peut être nul si c'est un like sur un message created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (userId) REFERENCES users(id), FOREIGN KEY (messageId) REFERENCES messages(id), FOREIGN KEY (courseId) REFERENCES courses(id) );